  ...
  const moderateUser = async (
    ...
    try {
      ...
      if (result.errors?.length)
        throw new Error(
          result.errors
            .map((operationError: any) => errorText(operationError))
            .join(', ')
        );
          ...
          {tab === 'recipes' && (
            <section className="ak-panel mt-6 p-6">
              ...
              <p className="mt-2 text-sm text-[var(--theme-text-muted)]">
                Transfer recipe ownership through the administrator-only backend
                operation. The original creator metadata is retained.
              </p>
              <div className="mt-4 grid gap-3 md:grid-cols-[1fr_1fr_auto]">
                <select
                  aria-label="Recipe to transfer"
                  value={transferRecipeId}
                  onChange={(event) => setTransferRecipeId(event.target.value)}
...
function Stat({
  ...
  return (
    <button
      ...
      className={`rounded-2xl border p-4 text-left transition ${active ? 'text-white shadow-lg' : 'border-[var(--theme-border)] bg-[var(--theme-surface)] hover:border-[var(--theme-accent)]'}`}
    >
      <p
      ...
      <p
        className={`mt-1 text-xs ${active ? 'text-white/80' : 'text-[var(--theme-text-muted)]'}`}
      >
...
function UserTable({
  ...
  return (
    <div className="mt-5 max-h-[32rem] overflow-auto rounded-2xl border border-[var(--theme-border)]">
      <table className="min-w-full text-left text-sm">
        ...
        <tbody>
          {users.map((user) => (
            <tr key={user.id} className="border-t border-[var(--theme-border)]">
